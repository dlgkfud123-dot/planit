...
export default function PlannerMap({stops,allDays,activeDay,activeIndex,onSelect}:Props){
  ...
  useEffect(()=>{if(!mapReady)return;let cancelled=false;void import("leaflet").then(({default:L})=>{const map=mapRef.current;if(!map||cancelled)return;markerLayerRef.current?.remove();const group=L.layerGroup().addTo(map),validStops=stops.filter(valid);markerLayerRef.current=group;markersRef.current=validStops.map((stop,index)=>L.marker([stop.lat,stop.lng],{icon:L.divIcon({className:"planPinShell",html:`<button class="planPin" aria-label="${escapeHtml(stop.name)}">${index+1}</button>`,iconSize:[44,44],iconAnchor:[22,22]})}).addTo(group).on("click",()=>selectRef.current(index)));const active=stopsRef.current[activeRef.current];if(active&&valid(active)){map.invalidateSize({pan:false});map.setView([active.lat,active.lng],15,{animate:false})}});return()=>{cancelled=true}},[stops,mapReady]);
